...
const Chatbot = () => {
  const [messages, setMessages] = useState([
    { text: "Hello, I'm Kairo. Your digital companion. How can I assist you today?", sender: "bot" },
  ]);
  ...
  const fetchGeminiResponse = async (userInput) => {
    if (!GEMINI_API_KEY) {
      ...
      setMessages(prev => [...prev, {
        text: "Configuration error: API key missing. Please make sure the Gemini API key is properly set in the environment variables.",
        sender: "bot"
      }]);
      return;
    }
    ...
    try {
      ...
      const botResponse = data.candidates[0]?.content?.parts?.[0]?.text || 
                         "I couldn't understand that response. Could you try again?";
      
      setMessages(prev => [...prev, { text: botResponse, sender: "bot" }]);
    } catch (error) {
      console.error("Error fetching Gemini response:", error);
      setMessages(prev => [...prev, {
        text: `Sorry, I'm having trouble responding. ${error.message}. Please try again later.`,
        sender: "bot"
      }]);
    } finally {
      setIsTyping(false);
  ...
  return (
    <ChatbotContainer>
      ...
      {!user ? (
        <SignInContainer>
          <SignInMessage>Sign in to start chatting with Kairo</SignInMessage>
...
const Message = styled.div`
  background-color: ${({ $sender }) => ($sender === "user" ? "#005c97" : "#2d2d42")};
  background: ${({ $sender }) => 
    $sender === "user" 
